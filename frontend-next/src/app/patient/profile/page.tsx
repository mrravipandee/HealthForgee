    ...
    const updateUserProfileData = async () => {
        try {
            ...
            console.log('Sending health data:', {
                bloodType: safeUserData.bloodType || 'Unknown',
                activityLevel: safeUserData.activityLevel || 'Sedentary',
                medicalCondition: safeUserData.medicalCondition || 'None',
                allergies: safeUserData.allergies || '',
                emergencyContact: safeUserData.emergencyContact || '',
                weight: safeUserData.weight || '',
                height: safeUserData.height || ''
            });

            const formData = new FormData();
            formData.append('userId', String(safeUserData._id || safeUserData.id || ''))
            formData.append('name', String(safeUserData.name || ''))
            formData.append('phone', String(safeUserData.phone || ''))
            formData.append('age', String(safeUserData.age || ''))
            formData.append('gender', String(safeUserData.gender || 'Prefer Not To Say'))
            formData.append('weight', String(safeUserData.weight || ''))
            formData.append('height', String(safeUserData.height || ''))
            formData.append('bloodType', String(safeUserData.bloodType || 'Unknown'))
            formData.append('activityLevel', String(safeUserData.activityLevel || 'Sedentary'))
            formData.append('medicalCondition', String(safeUserData.medicalCondition || 'None'))
            formData.append('allergies', String(safeUserData.allergies || ''))
            formData.append('emergencyContact', String(safeUserData.emergencyContact || ''))
            formData.append('otherMedicalCondition', String(safeUserData.otherMedicalCondition || ''))
            formData.append('otherActivityLevel', String(safeUserData.otherActivityLevel || ''))
                        ...
                        <div className='inline-block relative'>
                            ...
                            <div className='absolute inset-0 bg-black bg-opacity-40 rounded-full flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity duration-300'>
                                <div className='bg-white rounded-full p-2 shadow-lg'>
                                    <img className='w-6' src={assets.upload_icon.src} alt="Upload" />
                                </div>
                            </div>
                        </div>
